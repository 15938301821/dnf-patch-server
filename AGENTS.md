...
- 提供任意 Prompt、任意模型、任意工具调用或通用网络代理 API。
...
- 客户端 token 与 Worker token 必须使用不同值；模型密钥只从服务进程环境读取。任何凭据不得进入数据库、API、WebSocket、日志、错误或证据正文。
...
6. **测试**：为新增契约、状态转换、冲突、租约、Guardrail、证据归属和安全分支补充测试。数据库结构变更必须修改 Drizzle schema、生成并审查 migration。
