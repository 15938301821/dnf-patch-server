...
} from "../../../src/modules/job/profession-model-execution.js";
import { ProfessionModelExecutionRepository } from "../../../src/modules/job/profession-model-execution.repository.js";
import {
...
describe("ProfessionModelExecutionRepository transitions", () => {
  it("rejects a source visual Artifact whose finalized digest drifts", async () => {
    const harness = transitionHarness([
      [leasedJob()],
      [{ value: now }],
      [{ ...sourceVisualRow(), artifactSha256: "F".repeat(64) }],
    ]);
    ...
    ).resolves.toEqual({ status: "source-visual-input-mismatch" });
    expect(harness.insert).not.toHaveBeenCalled();
    expect(harness.update).not.toHaveBeenCalled();
  });

  it("refuses Engineer reservation before the same-attempt reference image passes", async () => {
    const harness = transitionHarness([
      [leasedJob()],
      [{ value: now }],
      [sourceVisualRow()],
...
function sourceVisualInput(): RequestProfessionSkillExecutionInput["sourceVisualInput"] {
  return {
...
function sourceVisualRow(): Record<string, unknown> {
  const provenance = {
    kind: "profession-source-visual-input-v2",
    schemaVersion: 2,
    jobId,
    runId,
    attempt: 2,
    ...
    frameManifestArtifactId:
      validPayload().parameters.promptPackage.skills[0]?.sourceEvidence
        .sourceFrameManifestArtifactId,
    frameManifestSha256: "F".repeat(64),
    frameMapSha256: sourceVisualInput().frameMapSha256,
    frameCount: 1,
  };
  return {
    artifactId: sourceVisualInput().artifactId,
    artifactStorageKey: "artifacts/source-visual.png",
    artifactLogicalName: "profession-source-visual-input.png",
    artifactMediaType: "image/png",
    artifactByteLength: sourceVisualInput().byteLength,
    artifactSha256: sourceVisualInput().sha256,
    artifactProvenance: provenance,
    sessionObjectKey: "artifacts/source-visual.png",
    sessionLogicalName: "profession-source-visual-input.png",
    sessionMediaType: "image/png",
    sessionByteLength: sourceVisualInput().byteLength,
    sessionSha256: sourceVisualInput().sha256,
