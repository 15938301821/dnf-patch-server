/**
 * @fileoverview 构造 Profession passed 接收测试使用的当前 V5 与历史 V3/V4 输出 provenance；
 ...
 * 副作用：无。安全边界：默认只能生成 schemaVersion 5 + quality V5；历史函数不能用于成功场景。
 */
import { skillProductionFixture } from "./patch-task-skill-production.fixture-ids.js";

/** 构造当前 projects V5 provenance，作为 passed 接收的质量事实源。 */
...
/** 构造当前 validation V5 provenance，并绑定同代 projects Artifact。 */
...
/** 构造当前 source preview V5 provenance，并绑定同代 validation Artifact。 */
...
/** 构造当前 result preview V5 provenance，并反向绑定同代 source preview。 */
...
> {
  return {
    ...
    referenceTransferQuality: historicalQualityV2(),
  };
}
...
> {
  return {
    ...
    referenceTransferQuality: historicalQualityV3(),
  };
}
