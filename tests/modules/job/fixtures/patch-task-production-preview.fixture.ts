/**
 * @fileoverview 构造浏览器 production 三证据预览测试使用的 creative V4 与历史 V3 provenance；
 * 不模拟数据库查询、Run 所有权、上传 finalize 或短期下载授权。
 * @module modules/job/patch-task-production-preview-fixture
 * @author AI生成
 * @created 2026-07-30
 * @relatedPlan N/A - creative-stable-frame 生产证据同步
 *
 * 调用关系：patch-task-reference-image.repository-support.spec 把本文件产物同时作为 upload session
 * 与 Artifact provenance。输入是测试中的 Job/skill/Artifact 身份，输出严格同代的 validation、
 * source 或 result JSON。副作用：无。安全边界：默认生成 V4/quality V4；旧 V4/quality V3 与
 * 历史 V3/quality V2 必须显式选择，禁止测试无意把历史质量当作当前生产默认值。
 */
...
/** 构造 validation provenance；generation 缺省时始终返回当前 V4。 */
export function createCreativeValidationProvenance(
  identity: ProductionPreviewEvidenceIdentity,
  generation: ProductionPreviewEvidenceGeneration = "current-v4",
...
/** 构造 source preview provenance；generation 缺省时始终返回当前 V4。 */
...
export function createCreativeResultPreviewProvenance(
  ...
  const generation = options.generation ?? "current-v4";
  const contract = generationContract(generation);
  return {
    ...creativeBaseProvenance(identity, generation),
...
/** 当前新 Run 对浏览器公开的 stable-frame quality V4。 */
...
function generationContract(
  ...
  return generation !== "historical-v3"
    ? {
        schemaVersion: 4,
