...
import type { EncryptedModelCredential } from "../../../src/modules/model-configuration/model-credential-cipher.js";
import type { SaveModelRoleConfigurationInput } from "../../../src/modules/model-configuration/model-configuration.contracts.js";
import type {
  ModelConfigurationRecord,
  ModelConfigurationWrite,
} from "../../../src/modules/model-configuration/model-configuration.repository.js";
import { ModelConfigurationService } from "../../../src/modules/model-configuration/model-configuration.service.js";
...
describe("ModelConfigurationService", () => {
  ...
  it("requires every role Key on first save and never returns submitted values", async () => {
    await expect(
      service.save(userId, {
        ...
        referenceGenerator: roleInput("image-v2", undefined, "default"),
      }),
    ).rejects.toBeInstanceOf(BadRequestException);

    const saved = await service.save(userId, {
      orchestrator: roleInput("planner-v2", "planner-key", "high"),
      spriteProcessor: roleInput("engineer-v2", "engineer-key"),
      referenceGenerator: roleInput("image-v2", "image-key", "default"),
    });
  ...
  it("keeps the encrypted Key when a later save leaves apiKey empty", async () => {
    await service.save(userId, {
      ...
      referenceGenerator: roleInput("image-v1", "image-key", "default"),
    });
  ...
  it("normalizes a legacy text default before returning call configuration", async () => {
    records.push({
      userId,
      role: "orchestrator",
      endpoint: "https://user-models.example/v1",
      ...
      reasoningEffort: "default",
      credential: credential("planner-key"),
      version: 1,
    });
    ...
    saveAll(requestedUserId: string, writes: ModelConfigurationWrite[]) {
      ...
      for (const write of writes) {
        ...
        if (existing) {
          ...
          existing.reasoningEffort = write.reasoningEffort;
          existing.version += 1;
...
function roleInput(
  model: string,
  apiKey?: string,
  reasoningEffort: SaveModelRoleConfigurationInput["reasoningEffort"] = "medium",
