...
import type {
  ...
  ProductionFixture,
  UploadFixture,
} from "./style-package-context.repository.spec-support.js";
import type {
...
function validUploads(): UploadFixture[] {
  ...
  const artifacts = validArtifacts();
  return [
    outputUpload(productions[0], artifacts[0], firstProjectsUploadId),
    outputUpload(productions[0], artifacts[1], firstValidationUploadId),
    outputUpload(productions[1], artifacts[2], secondProjectsUploadId),
    outputUpload(productions[1], artifacts[3], secondValidationUploadId),
  ];
}

function outputUpload(
...
function outputArtifact(
  ...
  const base = {
    ...
    safety: {
      referenceImageUsedForRuntimePixels: false as const,
  ...
  return {
    ...
    provenance:
      role === "projects"
        ? { ...base, kind: "profession-aseprite-projects-v1" as const }
        : {
            ...base,
            kind: "profession-aseprite-validation-v1" as const,
            asepriteProjects: {
