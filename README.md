...
- `npm run probe:profession-persistence -- <professionJobId> [runId]`：针对已经存在的本地 Profession Job 执行显式诊断。命令要求提供当前本地服务使用的 `DATABASE_URL`、`OBJECT_STORAGE_ENDPOINT`、`OBJECT_STORAGE_BUCKET`、`OBJECT_STORAGE_ACCESS_KEY`、`OBJECT_STORAGE_SECRET_KEY`，可沿用 `OBJECT_STORAGE_REGION` 与 `OBJECT_STORAGE_FORCE_PATH_STYLE`；数据库和对象存储端点都必须是回环地址。探针只读 MySQL 与既有 Artifact，按数据库中的对象引用、长度和 SHA-256 核验工程师计划、参考图及可选 Run 的技能 validation ZIP；另创建一个随机 `diagnostics/` 对象验证写入/读回，并在所有路径尝试删除，清理失败时返回非零退出码。它不会调用模型或 Worker、不会读取游戏目录、不会部署，也不证明客户端兼容或全技能覆盖；该命令访问现有本地数据面，不属于 `npm run gate`。
